...
void MotorDrv::L298N(int ltc, int rtc, int lpwm, int lpos, int lneg, int rpwm, int rpos, int rneg, float intensity ){
...
void MotorDrv::L298Nsingle(int L298Ntc, int pwm, int pos, int neg, float intensity ){
    intensity = (intensity * .01) ; // Turning intensity into an easy to use multiplier
    
    // Mapping motor movement to transmitter stick values.
    // Constraints help even out the motor performance when the robot is backing up. Without this constraint the motors stop behaving when I reverse the motor, especially at the lowest end.
    control = constrain(map(L298Ntc, 1040, 1881, -255, 255), round(-100*intensity), round(255*intensity) ) ;
    
    if(control > 10){
          digitalWrite(pos, HIGH) ;		// "+" on driver
          digitalWrite(neg, LOW) ;		// "-" on driver
          analogWrite(pwm, control) ;		// This is pulse width! Send to appropriate spot on driver or nothing will happen!
        }
      if(control < -10){
...
void MotorDrv::IBT2(int ls, int rs, int m1pwml, int m1pwmr, int m1enl, int m1enr, int m2pwml, int m2pwmr, int m2enl, int m2enr, float intensity ){
...
void MotorDrv::IBT2single(int tc, int pwml, int pwmr, int enl, int enr, float intensity ){

    intensity = (intensity * .01) ; // Turning intensity into an easy to use multiplier
    
    // Mapping motor movement to transmitter stick values.
    // Constraints help even out the motor performance when the robot is backing up. Without this constraint the motors stop behaving when I reverse the motor, especially at the lowest end.
    control = constrain(map(tc, 1040, 1881, -255, 255), round(-150*intensity), round(255*intensity) ) ;
	
	  // Forward movement (Motor #1)
      if(ls > 10){
          analogWrite(pwml, control) ; // This is pulse width! Send to appropriate spot on driver or nothing will happen!
          analogWrite(pwmr, 0) ;
